...
|-----------|--------|----------------------|------------------|----------------|---------------|-----------|
| Button | Stable | `primary`, `secondary`, `outline`, `ghost`; intent `default` / `danger`; sizes `sm`–`xxl` + icon sizes; `loading`; `fullWidth`; `disabled` | No dedicated `link` variant (docs “As Link” uses `buttonVariants` + Next `Link`) | Interaction grid exists. No `aria-disabled` story (native `disabled` only) | Focus ring, loading `aria-busy` | `Components/Button` — mapped |
