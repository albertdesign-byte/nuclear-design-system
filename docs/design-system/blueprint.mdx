...
**Lucide** — biblioteca única. Cuatro tamaños. Un stroke.

```
Grid base     24px    icon-xl — referencia, uso raro
Default       16px    icon-md — 90% del producto
Compact       14px    icon-sm — badges, xs button
Standalone    20px    icon-lg — empty state
...
| Dropdown Menu | Acciones, SplitButton | P0 |
...
| **ButtonGroup** | Button[] con spacing unificado | `@/components/composite/button-group` |
...
| Template | Patterns incluidos | Caso de uso |
|----------|-------------------|-------------|
| **DashboardLayout** | Sidebar + PageHeader + grid de StatCards + Charts | Panel principal del médico |
| **ListPageLayout** | PageHeader + SearchToolbar + DataTable + Pagination | Listado de pacientes, citas, órdenes |
| **DetailPageLayout** | PageHeader + Tabs + Card sections + Actions | Ficha de paciente, detalle de orden |
| **SettingsPageLayout** | Sidebar nav + FormLayout sections | Configuración de cuenta, preferencias |
| **ModalFormLayout** | Dialog + FormLayout + Submit/Cancel | Crear/editar entidades inline |

**Regla:** Los templates definen layout y slots, no contenido. El contenido lo provee la aplicación.
...
graph TD
    ...
    FormLayout --> ModalFormLayout
    Dialog --> ModalFormLayout
    Patterns --> Templates
```
...
|-----------|-----------|---------|----------|
| Button | IconButton | PageHeader | ListPage |
| Button | ButtonGroup | SearchToolbar | Dashboard |
| Input | InputGroup → SearchInput | SearchToolbar | ListPage |
| Select | FormField | FilterToolbar | ListPage |
| Checkbox | FormField | FilterToolbar | — |
| Card | StatCard | — | Dashboard |
| Table | — | DataTable | ListPage |
| Dialog | ConfirmDialog | — | ModalForm |
| Form | — | FormLayout | Settings, ModalForm |
| Tabs | — | — | DetailPage |
